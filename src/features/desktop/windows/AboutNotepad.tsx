...
type MarkerStyle = "yellow" | "pink";
type TextSegment = { text: string; bold?: boolean; marker?: MarkerStyle };
...
const STORY: TextSegment[][] = [
  [{ text: "Back in elementary school, my parents somehow put me in a " }, { text: "computer specialty class", bold: true, marker: "yellow" }, { text: " instead of something more expected like calligraphy or violin. I still don't know why." }],
  ...
  [{ text: "Somehow, I did. :)", bold: true, marker: "pink" }],
  ...
  [{ text: "Have fun exploring ", bold: true }, { text: "my little corner of the internet", bold: true, marker: "pink" }, { text: ". :)", bold: true }],
];
...
const deterministicFraction = (index: number, salt = 0) => {
  let value = Math.imul(index + 1 + salt, 0x45d9f3b);
  value = Math.imul(value ^ (value >>> 16), 0x45d9f3b);
  return ((value ^ (value >>> 16)) >>> 0) / 0xffffffff;
};

const delayBetween = (index: number, minimum: number, maximum: number, salt = 0) =>
  Math.round(minimum + deterministicFraction(index, salt) * (maximum - minimum));

const getTypingDelay = (typedLength: number) => {
  const previousCharacter = FULL_TEXT[typedLength - 1] ?? "";
  const upcomingText = FULL_TEXT.slice(typedLength);

  if (upcomingText.startsWith("\n\n")) {
    return delayBetween(typedLength, 450, 750, 11);
  }
  if (previousCharacter === "\n") {
    return delayBetween(typedLength, 8, 16, 13);
  }
  if (/[.!?]/.test(previousCharacter) || FULL_TEXT.slice(0, typedLength).endsWith(":)")) {
    return delayBetween(typedLength, 220, 420, 17);
  }
  if (previousCharacter === ",") {
    return delayBetween(typedLength, 100, 180, 19);
  }
  if (/[—:;]/.test(previousCharacter)) {
    return delayBetween(typedLength, 120, 250, 23);
  }
  if (previousCharacter === " ") {
    return delayBetween(typedLength, 7, 18, 29);
  }

  const burstPosition = typedLength % 23;
  const inFastBurst = burstPosition >= 5 && burstPosition <= 10;
  return delayBetween(typedLength, inFastBurst ? 22 : 28, inFastBurst ? 38 : 55, 31);
};
  ...
  return (
    <p className={className} aria-hidden="true">
      {segments.map((segment, segmentIndex) => {
        ...
        const highlightComplete = segment.marker && visibleCharacters === segment.text.length;
        const markerClassName = segment.marker
          ? `about-marker about-marker--${segment.marker}${highlightComplete ? " is-marked" : ""}${segment.text === "Somehow, I did. :)" ? " is-punchline" : ""}`
          : undefined;
        return segment.bold
          ? (
              <strong
  ...
  return (
    <p className={className}>
      {segments.map((segment, index) => segment.bold
        ? <strong key={index} className={segment.marker ? `about-marker about-marker--${segment.marker} is-marked` : undefined}>{segment.text}</strong>
...
export function AboutNotepad({ className = "", style, ...props }: PositionedWindowProps) {
  ...
  useEffect(() => {
    ...
    const delay = getTypingDelay(typedLength);
